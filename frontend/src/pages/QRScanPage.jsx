...
//         Scan a patient's QR code to access their critical medical information
//         for emergency care.
//       </p>
...
//           <li>
//             <strong>Scan QR Code:</strong> Point your camera at the patient's
//             PulseChain QR code
//           </li>
//           <li>
//             <strong>Upload Image:</strong> If you have an image of the QR code,
//             upload it here
//           </li>
//           <li>
//             <strong>Enter ID:</strong> If you know the patient's ID, enter it
//             directly
//           </li>
//         </ul>
//       </div>
...
function QRScanPage() {
      ...
      interval = setInterval(async () => {
        ...
        if (code?.data) {
          ...
          try {
            ...
            if (token) {
              navigate(`/emergency/access?token=${token}`);
            } else {
              setError("Token not found in QR code");
            }
          } catch (err) {
            console.error("Invalid QR code content", err);
            setError("Scanned QR code is not a valid URL");
  ...
  return (
    <div className="py-12 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto">
      ...
      {/* Error message */}
      {error && (
        <div className="flex items-center justify-center mb-6 max-w-md mx-auto p-3 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 rounded-lg">
          <FiAlertCircle className="h-5 w-5 mr-2" />
          <span>{error}</span>
        </div>
      )}
            ...
            <form onSubmit={handlePatientIdSubmit}>
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={(e) => setPatientId(e.target.value)}
                  placeholder="Enter patient ID (e.g., user-123)"
                  className="w-full px-3 py-2 border border-neutral-300 dark:border-neutral-600 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 dark:bg-neutral-700 dark:text-white"
                />
              </div>
              <div className="text-center">
                <button
